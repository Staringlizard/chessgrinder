...
function NewHub() {
    ...
    hub.opponent_color = "play_black";                  // Selected color for engine to play

    hub.seed = Date.now()%4645455524863;
    hub.repeat = TIMES_REPEAT_OPENING;

    hub.looker.add_to_queue(hub.tree.node.board);       // Maybe make initial call to API such as ChessDN.cn...
    Object.assign(hub, hub_props);

    hub.load_polyglot_book("C:\\books\\titans.bin");
...
let hub_props = {
    ...
    behave: function(reason) {                          // reason should be "position" or "behaviour"
        //console.log(`debug: behave (${config.behaviour})`);
    ...
    set_behaviour: function(s) {
        ...
        //console.log(`debug: set_behaviour (${s})`);
    ...
    maybe_setup_book_move: function() {
        ...
        //console.log(`debug: maybe_setup_book_move`);

        if (!this.book || this.tree.node.terminal_reason()) {
            this.err_receive(`<span class="blue">no book active!</span>`, true);
            return false;
        }

        if (typeof config.book_depth === "number" && this.tree.node.depth >= config.book_depth * 2) {
            this.err_receive(`<span class="blue">book depth reached!</span>`, true);
            setTimeout(() => {
                config.book_explorer = false;
                this.new_game();
                this.set_behaviour(hub.opponent_color);
                this.behave("behaviour");
            }, 2000);
            return false;
        }
        ...
        if (total_weight <= 0) {
            // No book moves for previous position!
            this.err_receive(`<span class="blue">Congrats, no more move found!.</span>`, true);
            setTimeout(() => {
                config.book_explorer = false;
                this.new_game();
                this.set_behaviour(hub.opponent_color);
    ...
    draw_fantasy_from_moves: function(moves) {
        ...
        //console.log(`debug: draw_fantasy_from_moves`);
    ...
    draw_explorer_arrows: function() {
        ...
        //console.log(`debug: draw_explorer_arrows`);
    ...
    receive_bestmove: function(s, relevant_node) {
        ...
        //console.log(`debug: receive_bestmove`);
    ...
    err_receive: function(s, clear) {
        ...
        this.info_handler.err_receive(s, clear);
    },
    ...
    play_info_index: function(n) {
        //console.log(`debug: play_info_index`);
    ...
    new_game: function() {
        ...
        hub.repeat--;
        if(hub.repeat == 0) {
            console.log("choose different seed!");
            hub.repeat = TIMES_REPEAT_OPENING;
            RandSeed(true);
        } else {
            RandSeed(false);
    ...
    boardfriends_click: function(event) {
        ...
        //console.log(`debug: boardfriends_click`);
    ...
    promotiontable_click: function(event) {
        //console.log(`debug: promotiontable_click`);
    ...
    handle_drop: function(event) {
        ...
        if (text_data.startsWith("overlay_")) {
            ...
            // MATHIAS: check if player did book move
            {
                let move = source.s + dest.s;
                let is_book_move = false;
                ...
                if(objects.length == 0) {
                    this.err_receive(`<span class="blue">Congrats, no more move found!.</span>`, true);
                    console.log(`computer had the last move, now there are non left!`);
                    setTimeout(() => {
                        config.book_explorer = false;
                        this.new_game();
                        this.set_behaviour(hub.opponent_color);
                ...
                if (Array.isArray(objects)) {
                    for (let o of objects) {
                        //this.err_receive(`<span class="blue">comparing ${move} with ${o.move}</span>`);
                        if (move === o.move) {
                            this.err_receive(`<span class="blue">book move!</span>`, true);
                            is_book_move = true;
                            //break;
                        }
                    }
                ...
                if (is_book_move === false) {
                    this.err_receive(`<span class="blue">You did not play book move silly!!!!</span>`, false);
                    console.log(`you bad!`);
                    for (let o of objects) {
                        this.err_receive(`<span class="blue">these are the book moves: ${o.move}</span>`);
                    }

                    config.book_explorer = true;
                    setTimeout(() => {
                        config.book_explorer = false;
                        this.new_game();
                        this.set_behaviour(hub.opponent_color);
            ...
            if (source && dest) {
                ...
                // MATHIAS START Check again, after a move has been made to see if there are any book moves left
                if (ok) {
                    let objects = BookProbe(KeyFromBoard(this.tree.node.board), this.book);
                    let total_weight = 0;

                    //console.log(`object_lenght after move: ${objects.length}`);
                    ...
                    if (total_weight <= 0) {
                        this.err_receive(`<span class="blue">Congrats, no more move found!.</span>`, true);
                        console.log(`I had the last move, now there are non left!`);
                        setTimeout(() => {
                            config.book_explorer = false;
                            this.new_game();
                            this.set_behaviour(hub.opponent_color);
            ...
            if (hub.opponent_color === "play_black") {
                this.set_behaviour("play_black");
            } else {
                this.set_behaviour("play_white");
            }
    ...
    toggle_flip: function() {                       // config.flip should not be directly set, call this function instead.
        ...
        if (hub.opponent_color === "play_white") {
            hub.opponent_color = "play_black";
            this.set_behaviour("play_black");
        } else {
            hub.opponent_color = "play_white";
            this.set_behaviour("play_white");
        }

        console.log(`toggle_flip (${hub.opponent_color})`);
